...
const getDockerRunCommand = () =>
  ` stop ${dockerName} || docker run --name ${dockerName} -it --rm -d -v $(pwd)/node_modules/@mihanizm56/webpack-stream-watcher/config/certs:/certs -v $(pwd)/node_modules/@mihanizm56/webpack-stream-watcher/config/nginx.conf:/etc/nginx/nginx.conf -v $(pwd)/build:/usr/share/metadata/core/build -p ${dockerPort}:443 fholzer/nginx-brotli nginx -g 'daemon off;'`;

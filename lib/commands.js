...
const getDockerRunCommand = () =>
  `$(pwd)/build/runtime-config.js && bash $(pwd)/config/deploy/runtime-envs/local/make-runtime-config-local.sh && docker stop ${dockerName} || docker run --name ${dockerName} -it --rm -d -v $(pwd)/config/test-certs:/certs -v $(pwd)/config/deploy/nginx.conf:/etc/nginx/nginx.conf -v $(pwd)/build:/usr/share/metadata/core/build -p ${dockerPort}:443 fholzer/nginx-brotli nginx -g 'daemon off;'`;
